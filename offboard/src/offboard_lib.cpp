...
void OffboardControl::position_control(ros::NodeHandle nh, ros::Rate rate)
{
    ...
    takeOff(rate);
		...
		while (ros::ok())
        {
            ...
            bool check = (check_error_.data < 0.1) ? true:false;
            std::cout << "\n" << check << std::endl;
            if(check)
            {
                ...
                hover(targetTransfer(target_pub_pose.pose.position.x, target_pub_pose.pose.position.x, z_target), rate);
                landing(targetTransfer(target_pub_pose.pose.position.x, target_pub_pose.pose.position.x, z_target), rate);
                break;
